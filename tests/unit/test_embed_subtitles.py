...
def test_embed_subtitles():
    ...
    # 检查embed_subtitles_to_video方法是否存在
    if not hasattr(processor, 'embed_subtitles_to_video'):
        print("❌ VideoProcessor 缺少 embed_subtitles_to_video 方法")
        return False
    ...
    # 测试不存在的视频文件
    result = processor.embed_subtitles_to_video(
...
success = processor.embed_subtitles_to_video(
    'your_video.mp4',
    'your_subtitle.srt', 
    'output_with_embedded_subs.mp4'
)
print(f"软字幕嵌入{'成功' if success else '失败'}")
""")
    
    return True

if __name__ == "__main__":
    test_embed_subtitles()
